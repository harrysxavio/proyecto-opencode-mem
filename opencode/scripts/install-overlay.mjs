...
import { buildSkillRegistry } from "../../codex/scripts/skill-registry-generate.mjs";

const overlayFiles = [
  {
    source: "opencode/manager.template.md",
...
export async function buildOpenCodeOverlayPlan(options = {}) {
  const target = validateOpenCodeOverlayTarget(options.target ?? path.join(process.env.USERPROFILE ?? process.env.HOME ?? ".", ".config", "opencode"));
...
export async function installOpenCodeOverlay(options = {}) {
  const plan = await buildOpenCodeOverlayPlan(options);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const target = explicitTarget ?? path.join(process.env.USERPROFILE ?? process.env.HOME ?? ".", ".config", "opencode");
  try {
    if (!dryRun && !explicitTarget) {
      throw new Error("Real install requires explicit --target to avoid accidental writes");
    }
    const result = await installOpenCodeOverlay({ target, dryRun });
    console.log(`${dryRun ? "DRY RUN " : ""}OpenCode overlay install plan`);
